...
class GoBoard(object):
    ...
    def _diag_neighbors(self,point):
        ...
        #row,col = self._point_to_coord(point)
        #if 0 <= row <= self.size+1 and 0 <= col <= self.size+1:
        return self._border_removal([point-self.NS-1, point-self.NS+1, point+self.NS-1, point+self.NS+1])
        #else:
        #    raise ValueError("This point is out of range!")
    ...
    def _liberty_point(self, point, color):
        ...
        liberty: int
             Number of liberty that the given point has
        liberty_loaction: list
             Locations of liberties
        """
            ...
            for n in neighbors:
                if n not in met_points:
                    ...
                    if self.board[n]==color: 
                        ...
                        liberty+=1
                        liberty_location.append(n)
                    met_points.append(n)
        return (liberty, liberty_location)
        
    def _flood_opponents(self,board,color):
        """
        This function find the opponents of flood filled board.
        Arguments
        ---------
        board : numpy array
        ...
        ---------
        list:
             list of opponents surrounding flood
        """
        #cond = board==FLOODFILL
        #inds = np.where(cond)
        inds = list(*np.where(board == FLOODFILL))
        surrounding_opponents = set()
