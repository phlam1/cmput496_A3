...
class GoBoardUtil(object):
    ...
    def playGame(board, color, **kwargs):
        ...
        atari_defence = kwargs.pop('ataridefence', True)
        atari_capture = kwargs.pop('ataricapture', True)
        pattern = kwargs.pop('pattern', True)
        if kwargs:
            raise TypeError('Unexpected **kwargs: %r' % kwargs)
        numPass = 0
        for _ in range(limit):
            move = GoBoardUtil.generate_move_with_filter(board,pattern,check_selfatari, atari_defence, atari_capture)
            if move != None:
    ...
    def generate_atari_capture_moves(board):
        ...
        numliberty, last_liberty_point = board._liberty_point(last_move, GoBoardUtil.opponent(color))
        
        
        moves = []
        #Consider capture if there is ONLY 1 liberty
        if numliberty == 1:
            
            
            #Loops through all liberty positions (which there should only be 1)
            for move in last_liberty_point:
                cboard = board.copy()
                #Try to play
                (status, msg) = cboard._play_move(move, color)
                if status == True:
                    #If play is successful return move
                    moves.append(move)
    ...
    @staticmethod
    def generate_all_policy_moves(board,pattern,check_selfatari=True, atari_defence=True, atari_capture=True):
        """
        ...
        """
        if atari_capture:
            pattern_moves = GoBoardUtil.generate_atari_capture_moves(board)
        ...
        if atari_defence:
            pattern_moves = GoBoardUtil.generate_atari_defense_moves(board)
            if len(pattern_moves) > 0:
                pattern_moves = GoBoardUtil.filter_moves(board, pattern_moves, check_selfatari)
                if len(pattern_moves) > 0:
    ...
    @staticmethod
    def generate_move_with_filter(board, use_pattern, check_selfatari, atari_defence, atari_capture):
        """
            Arguments
            ---------
